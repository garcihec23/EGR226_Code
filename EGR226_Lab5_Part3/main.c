...
int SwitchBounce() {
    ...
    // Incorporates the second button
...
void init_ports() {
    ...
    P2->DIR &= ~BIT6;         // Setting the direction of Port 2 Pin 4 to be an input
        ...
        case OFF:
            P2->OUT &= ~7;                // Initialized with the LED off
                if (SwitchBounce()) {
                    ...
                    if(but_flag)
                    {
                    state = BLUE;        // if the switch button IS pressed, move to the RED state
        ...
        case RED:
            ...
            P2->OUT |= 1;                 // Turns on BIT0 (Red LED on)
                if (SwitchBounce()) {
                    if(!but_flag)
                    {
                    state = GREEN;        // if the switch button IS pressed, move to the RED state
